...
const columns: TableColumnsType<DataType> = [
  ...
  },
  {
    ...
    width: 90,
    render: (row) => (
      <a href={`/admin/updateProfile?id=${row.id}`}>
        <FileOutlined />
      </a>
    ),
  },
    ...
    <div className="min-h-[80vh] flex gap-3">
      ...
      <div className="w-[80%] px-10 overflow-y-auto flex flex-col gap-3">
        <div className="flex justify-between">
          ...
          <Link
            to={`/admin/addUser`}
            className="border border-gray-300 px-3 py-1 rounded-xl hover:border-black transition-all"
          >
            +Add User
          </Link>
        </div>
        ...
        <div className="flex justify-center mt-3">
          <Pagination
            ...
            total={((getAllUser.data?.count ?? 0) / 5) * 10}
