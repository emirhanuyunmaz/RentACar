...
export class UserInteractor implements IUserInteractor{
    ...
    async loginUser({ email, password }: { email: string; password: string; }): Promise<String | undefined> {
        const data = await this.repository.loginUser({email,password})
        if (data){
            console.log("data",data);
            
            const userToken = this.token.createToken({id:data.id!,admin:data.isAdmin})
            return userToken
        }
        return undefined
    }
